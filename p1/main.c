...
void hist(char * argmnt[], head_t * lista, int aux){
    ...
    if (argmnt[1] == NULL){
        printList(lista, INT_MAX);
    }
    else if (strcmp(argmnt[1], "-c") == 0){
        clearList(lista);
    }
    else if (atoi(argmnt[1]+1) > 0){
        printList(lista, atoi(argmnt[1]+1));
    }
    else{
...
void comando(char * argmnt[], head_t * lista, int aux){
    ...
    if (argmnt[1] == NULL){
        ...
        strcpy(aux, getNelement(lista, atoi(argmnt[1]) - 1));
        processComand(lista, aux);
        printf("\n");
    }
...
void borrar(char * argmnt[], int aux, int op){                              /*Borrar recursivo*/
    if (op == 1){
    if(aux > 1){
...
bool processComand(head_t * lista, char * petition){
    
    time_t t = time(NULL);
    struct tm tm = *localtime(&t);
    char petitionAux[strlen(petition)];
    char * argument [MAX_CADENA];
    char Command [MAX_CADENA];

    strcpy(petitionAux, petition);
    
    int aux = CommandSplit(petition, argument);   

    if (strcmp(petition, "command") == 0 || strcmp(petition, "hist") == 0 || strcmp(petition, "\n")==0); // Evitar bucle infinito con command 
                                                                                                         // no registrar el comando hist/command y saltos de linea al ser llamado        
    else Insert(lista, petitionAux);
    ...
    if(strcmp(petition, "\n")==0){   // Entradas vacías a la terminal no implica un error
        ...
        if (endConditioning(Command)) {
            ...
            infosis(argument, aux);
             ...
             // PREGUNTAR SI HACE FALTA PONER O NO CONDICIONANTE A FGETS (==NULL) PARA EVITAR ERRORES
        else if (strcmp(Command, "ayuda") == 0){
            ayuda(argument, aux);
        }
        else if (strcmp(Command, "hist") == 0){
            hist(argument, lista, aux);
        }
        else if (strcmp(Command, "comando") == 0){
            comando(argument, lista, aux);
        }
...
int main(){
    ...
    end = processComand(lista, petition);
